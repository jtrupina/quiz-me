...
module.exports.sockets = {
  ...
  afterDisconnect: function (session, socket, cb) {
    ...
    if(typeof session.user !== 'undefined') {
      ChatConnectionService.removeUser(session.user);

      console.log("BROADCAST ZA 3SEC");
      setTimeout(function () {
        if(typeof session.user != 'undefined' && ! ChatConnectionService.isOnline(session.user)) {
          sails.sockets.broadcast('chat_users_online', 'user_left_chat', session.user);
          session.user = null;
        }
      }, 3000);
    }
